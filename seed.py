...
What's seeded (matches schema.prisma exactly):
    ...
    employees              — admin, jane, john  (all fields incl. date_of_birth)
...
# ─────────────────────────────────────────────────────────────────────────────
def uid() -> str:
    """Generate a fresh UUID string for rows that don't need a fixed ID."""
...
async def seed_status_master():
    ...
    rows = [
        # status_id,           code,             name,       entity,        description
    ...
    for status_id, code, name, entity, desc in rows:
        await db.status_master.create(data={
            ...
            "entity_type": entity,
            "description": desc,
            # created_by / updated_by are nullable — no employee exists yet
            "updated_at":  NOW,
        })
...
# ─────────────────────────────────────────────────────────────────────────────
# ROLES  — reviewer_weight populated per role
# ─────────────────────────────────────────────────────────────────────────────
async def seed_roles():
    print("👥 Seeding roles...")

    # (code, name, description, reviewer_weight)
    roles = [
...
# ─────────────────────────────────────────────────────────────────────────────
# DEPARTMENT TYPES & DEPARTMENTS  — created_by/updated_by set after admin exists
# ─────────────────────────────────────────────────────────────────────────────
async def seed_departments():
    """
    Seed departments WITHOUT created_by/updated_by first — admin doesn't exist yet.
    Call backfill_audit_fields() after all employees are created to fill them in.
    """
    ...
    for type_id, name, code in [
        ...
        await db.department_types.create(data={
            ...
            "type_name":          name,
            "type_code":          code,
            # created_by / updated_by are nullable — backfilled after admin exists
            "updated_at":         NOW,
        })
...
async def seed_designations():
    """
    Seed designations WITHOUT created_by/updated_by — backfilled after admin exists.
    """
...
async def seed_points_config():
    ...
    configs = [
        (PC_BASE_ID, "BASE_POINTS_PER_STAR", "2.200000",
         "Base points awarded per star in a review rating", date(2024, 1, 1)),
        (PC_MAX_ID,  "MAX_POINTS_PER_REVIEW", "50.000000",
         "Maximum points that can be awarded from a single review", date(2024, 1, 1)),
    ]
...
async def seed_employees():
    ...
    employees = [
        # (id, username, email, designation, dept, manager, doj, dob, avail_pts, redeemed, total, version)
        (
            ADMIN_ID, "admin.user", "admin@company.com",
            ADMIN_DESIG_ID, HR_DEPT_ID, None,
            date(2020, 1, 1), date(1985, 3, 15),
            999999, 0, 999999, 1,
        ),
        (
            JANE_ID, "jane.smith", "jane.smith@company.com",
            MGR_DESIG_ID, ENG_DEPT_ID, None,
            date(2022, 1, 1), date(1990, 6, 20),
            5000, 0, 5000, 1,
        ),
        (
            JOHN_ID, "john.doe", "john.doe@company.com",
            SR_DEV_DESIG_ID, ENG_DEPT_ID, JANE_ID,
            date(2024, 1, 15), date(1995, 11, 8),
            1500, 500, 2000, 5,
        ),
    ]
    ...
    for (
        ...
        desig_id, dept_id, manager_id,
        doj, dob,
        avail_pts, redeemed_pts, total_pts, version,
    ) in employees:
        await db.employees.create(data={
            ...
            **({"manager_id": manager_id} if manager_id else {}),
            ...
            # date_of_joining — @db.Date, pass a plain datetime (date part only)
            "date_of_joining": datetime.combine(doj, datetime.min.time()),
            # date_of_birth   — @db.Date, optional
        ...
        print(f"   ✅ {username:15s}  dob={dob}  wallet: {avail_pts} pts available")
    print()
...
async def seed_employee_roles():
    ...
    for emp_id, role_code in assignments:
        ...
        emp_name = {ADMIN_ID: "admin.user", JANE_ID: "jane.smith", JOHN_ID: "john.doe"}[emp_id]
        print(f"   ✅ {emp_name:15s} → {role_code}")
    print()
...
async def seed_reward_catalog():
    ...
    items = [
        # (name, code, desc, cat_id, default_pts, min_pts, max_pts, stock)
    ...
    for name, code, desc, cat_id, default_pts, min_pts, max_pts, stock in items:
        item = await db.reward_catalog.create(data={
            ...
            "max_points":     max_pts,
            "is_active":      True,
            # available_stock has @default(0) — set via raw SQL below
            "category_id":    cat_id,
            "created_by":     ADMIN_ID,
            "updated_by":     ADMIN_ID,
            "updated_at":     NOW,
...
async def seed_reviews():
    ...
    await db.reviews.create(data={
        ...
        "raw_points":  17.6,     # frozen at write time
        "created_by":  JANE_ID,
        "updated_by":  JANE_ID,
        "updated_at":  NOW,
    })

    # review_category_tags — multiplier_snapshot and category_code_snapshot
    # are frozen at write time (see schema comments)
    tags = [
...
# ─────────────────────────────────────────────────────────────────────────────
# TRANSACTIONS  (credit to john's wallet)
# ─────────────────────────────────────────────────────────────────────────────
async def seed_transactions():
    ...
    txn_data = [
        # (amount, type_id, status_id, description, ref, at_offset_days)
    ...
    for amount, type_id, status_id, desc, ref, offset in txn_data:
        txn_at = NOW + timedelta(days=offset)
...
# ─────────────────────────────────────────────────────────────────────────────
# REWARD HISTORY  (john redeems a hoodie)
# ─────────────────────────────────────────────────────────────────────────────
...
async def seed_audit_log():
    ...
    entries = [
        # (table, record_id, operation, old_values_dict_or_None, new_values_dict, performed_by)
        (
            "employees", JOHN_ID, "INSERT",
            None,
            {"username": "john.doe", "status": "ACTIVE"},
            ADMIN_ID,
        ),
        (
            "wallets", JOHN_ID, "UPDATE",
        ...
        (
            "reviews", uid(), "INSERT",
            None,
            {"reviewer": "jane.smith", "receiver": "john.doe", "rating": 4},
            JANE_ID,
        ),
    ]

    # Use raw SQL — Prisma-Python's handling of nullable Json columns with
    # the ORM create() API is inconsistent across client versions.
...
async def main():
    ...
    try:
        ...
        # ── Phase 1: pure lookup tables (zero employee FKs) ────────────────
        await seed_status_master()
        await seed_transaction_types()
        await seed_roles()

        # ── Phase 2: tables with nullable created_by — seed WITHOUT it first
        # (created_by / updated_by are nullable in the schema, so this is valid)
        await seed_departments()        # needs status_master done; no employee FK yet
        await seed_designations()       # no employee FK yet

        # ── Phase 3: all employees (admin first, then the rest) ────────────
        # Admin can now reference HR_DEPT_ID and ADMIN_DESIG_ID which exist.
        await seed_employees_admin_only()
        await seed_employees_rest()

        # ── Phase 4: backfill created_by / updated_by now admin exists ─────
        await backfill_audit_fields()

        # ── Phase 5: everything else that references employees ─────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# BACKFILL created_by / updated_by on tables seeded before admin existed
# ─────────────────────────────────────────────────────────────────────────────
async def backfill_audit_fields():
    """
    departments, department_types, and designations were seeded without
    created_by/updated_by because admin didn't exist yet.  Now that admin
    exists we fill those fields in via raw SQL (fastest, one query per table).
    """
    print("🔧 Backfilling created_by / updated_by on early tables...")

    tables = ["department_types", "departments", "designations"]
    for table in tables:
        await db.execute_raw(
            f"UPDATE {table} "
...
async def seed_employees_admin_only():
    """Seed admin first — needed so department/designation created_by FKs work."""
    ...
    await db.employees.create(data={
        ...
        "date_of_birth":   datetime.combine(date(1985, 3, 15), datetime.min.time()),
        "created_by":      ADMIN_ID,
        "updated_by":      ADMIN_ID,
        "updated_at":      NOW,
    })
    ...
    print("   ✅ admin.user  (dob=1985-03-15)\n")


async def seed_employees_rest():
    """Seed jane and john after departments and designations exist."""
    ...
    await db.employees.create(data={
        ...
        "status_id":       ACTIVE_STATUS_ID,
        "date_of_joining": datetime.combine(date(2022, 1, 1), datetime.min.time()),
        "date_of_birth":   datetime.combine(date(1990, 6, 20), datetime.min.time()),
        "created_by":      ADMIN_ID,
        "updated_by":      ADMIN_ID,
        "updated_at":      NOW,
    })
    ...
    print("   ✅ jane.smith  (dob=1990-06-20)")
    ...
    await db.employees.create(data={
        ...
        "manager_id":      JANE_ID,
        "status_id":       ACTIVE_STATUS_ID,
        "date_of_joining": datetime.combine(date(2024, 1, 15), datetime.min.time()),
        "date_of_birth":   datetime.combine(date(1995, 11, 8), datetime.min.time()),
        "created_by":      ADMIN_ID,
        "updated_by":      ADMIN_ID,
        "updated_at":      NOW,
    })
