"""
conftest.py — shared fixtures and stubs for wallet tests.
"""
import sys
import types
import pytest
...
mock_db.transaction_types.find_many = AsyncMock()
mock_db.wallets.find_unique = AsyncMock()
mock_db.transaction_types.find_unique = AsyncMock()
...
# Stub recognition points engine
_points_engine_stub = types.ModuleType("src.recognition.points_engine")
sys.modules["src.recognition.points_engine"] = _points_engine_stub
_points_engine_stub.calculate_points = MagicMock()
...
@pytest.fixture
def admin_user():
    return CurrentUser(id="admin-1", email="admin@test.com", roles=["SUPER_ADMIN"])
